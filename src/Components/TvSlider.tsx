...
function TvSlider({ category, data }: IData) {
  ...
  const increaseIndex = () => {
    if (data) {
      ...
      setBack(false);
      toggleLeaving();
      const totalTvLength = data.results.length;
      const maxIndex = Math.floor(totalTvLength / offset) - 1;
            ...
            {[...tvList]
              ...
              .map((tv) => (
                <Box
                  ...
                  onClick={() => onBoxClicked(category, tv.id)}
